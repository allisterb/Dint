...
public abstract class Runtime
{
    ...
    #region Properties
    public static bool RuntimeInitialized { get; private set; }
    ...
    public static string DintDevDir => Path.Combine(AppDataDir, "Dint");
    ...
    public static string RunFile => DintDevDir.CombinePath(ToolName + ".run"); 
    #endregion

    #region Methods
    public static void Initialize(string toolname, string logname, bool debug = false, bool enableLog = true, bool logToConsole = false)
    {
        lock (__lock)
        {
            ...
            if (enableLog)
            {
                if (!IsUnitTestRun)
                {
                    LoggerConfiguration = LoggerConfiguration.WriteTo.File(fulllogfilename);
                }
                if (debug)
                {
            ...
            {
                Info("{0} initialized from entry assembly {1}...", ToolName, EntryAssembly?.GetName().FullName ?? "(none)");
            }
        }
    }
    ...
    [DebuggerStepThrough]
    public static Operation? Begin(string messageTemplate, params object[] args) => Logger?.BeginOperation(messageTemplate, args);
